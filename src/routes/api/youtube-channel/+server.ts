...
async function fetchChannelBy(
	handleName: string
): Promise<{ channelId: string; latestVideos: object[] } | undefined> {
	let channelPageHTML;
	try {
	...
	const rssFeedURL =
		/https:\/\/www\.youtube\.com\/feeds\/videos\.xml\?(?:channel_id|user)=[0-9a-zA-Z_\-]+/.exec(
			channelPageHTML
		);
	if (rssFeedURL == null) {
	...
	return {
		...
		latestVideos: createLatestVideoEntries(rssXML.feed.entry)
	};
}

function createLatestVideoEntries(entry: unknown) {
	const createModel = (item: any) => ({
		videoId: item['yt:videoId'],
		title: item['media:group']['media:title'],
		thumbnailUrl: item['media:group']['media:thumbnail']['@_url'],
		description: item['media:group']['media:description'],
		viewCount: item['media:group']['media:community']['media:statistics']['@_views']
	});
	...
	if (Array.isArray(entry)) {
		return entry.map(createModel);
	}
	return [createModel(entry)];
}
