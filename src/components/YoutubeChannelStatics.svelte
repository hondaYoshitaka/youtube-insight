<script>
	...
	$: averageEngagementCount =
		videos.length > 0 ? (totalLikeCount + totalCommentCount) / videos.length : 0;
	$: averageEngagementRate =
		totalViewCount > 0 ? (totalLikeCount + totalCommentCount) / totalViewCount : 0;
...
<div>
	...
	<div class="mb-4">
		<div class="mb-4">
			<label for="handle-name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
				>チャンネル</label
			>
			<div class="relative">
		...
		<div class="mb-4">
			<label for="video-type" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
				>動画の種類</label
			>
			<div class="flex">
				<div class="flex items-center mr-4">
					...
					<label
						for="youtube-video"
						class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Youtube 動画</label
					>
				</div>
				<div class="flex items-center mr-4">
					<input
	...
	<section class="flex flex-wrap">
		...
		<aside class="w-1/5">
			...
			<ul class="mb-4">
				<li>
					合計: {totalViewCount.toLocaleString()} 回視聴 / 平均: {Math.round(
						averageViewCount
					).toLocaleString()}
					回視聴
				</li>
				<li>
					合計: {totalLikeCount.toLocaleString()} likes / 平均: {Math.round(
						averageLikeCount
					).toLocaleString()} likes
				</li>
				<li>
					合計: {totalCommentCount.toLocaleString()} comments / 平均: {Math.round(
						averageCommentCount
					).toLocaleString()}
					comments
				</li>
				<li>
					平均エンゲージメント
					{Math.round(averageEngagementCount).toLocaleString()} 回 ({Math.round(
						averageEngagementRate * 10_000
					) / 100} %)
				</li>
			</ul>
		</aside>
